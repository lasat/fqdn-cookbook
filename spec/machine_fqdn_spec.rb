...
describe 'MachineFqdn' do
  ...
  # four dots
  context 'there are four dots' do
    context 'machine_fqdn_as_hostname set to true' do
      before do
        node = Chef::Node.new
      ...
      it 'should set everything to the correct values' do
        expect(@machine_fqdn.fqdn).to eq('middle.earth.working.computers.biz')
        expect(@machine_fqdn.hostname).to eq('middle.earth.working.computers.biz')
        expect(@machine_fqdn.dnsdomainname).to eq('earth.working.computers.biz')
      end
    end
    ...
    context 'machine_fqdn_as_hostname set to false' do
      ...
      it 'should set everything to the correct values' do
        expect(@machine_fqdn.fqdn).to eq('middle.earth.working.computers.biz')
        expect(@machine_fqdn.hostname).to eq('middle')
        expect(@machine_fqdn.dnsdomainname).to eq('earth.working.computers.biz')
      end
    end
  end
